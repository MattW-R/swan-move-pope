...
describe("rent helpers", () => {
	describe("calculateAverageRentByRegion", () => {
		...
		it("does not throw for regions with properties", () => {
			...
			expect(() =>
				calculateAverageRentByRegion(properties, "N.IRELAND"),
			).not.toThrow();
			expect(() =>
				calculateAverageRentByRegion(properties, "WALES"),
			).not.toThrow();
		});
		...
		it("does throws error for regions without properties", () => {
			...
			expect(() =>
				calculateAverageRentByRegion(properties, "ENGLAND"),
			).toThrowError("No properties found for region ENGLAND");
		});
	});
	...
	describe("calculateMonthlyRentPerTenant", () => {
		...
		it("throws error if number of tenants is zero or less", () => {
			const property = { monthlyRentPence: 1000, capacity: 4 } as Property;
			expect(() =>
				calculateMonthlyRentPerTenant(property, 0, "pounds"),
			).toThrowError("Number of tenants must be greater than zero");
		});

		it("throws error if number of tenants exceeds property capacity", () => {
			const property = { monthlyRentPence: 1000, capacity: 4 } as Property;
			expect(() =>
				calculateMonthlyRentPerTenant(property, 5, "pounds"),
			).toThrowError("Number of tenants exceeds property capacity");
		});
	});
});
